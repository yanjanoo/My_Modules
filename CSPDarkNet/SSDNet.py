from src.CSPDarkNet import CSPDarkNet53
...
class CSPBackbone(nn.Module):
    def __init__(self,pretrainedWeight=False):
        super(CSPBackbone, self).__init__()
        self.net = CSPDarkNet53(pretrainedWeight)

        #self.out_channels = [1024, 512, 512, 256, 256, 256]
        self.out_channels = [256, 512, 1024, 512, 256, 256]
        # self.feature_extractor = nn.Sequential(*list(net.children()))
        #
        # block4_down_sample = self.feature_extractor[4].downSample
        # block5_down_sample = self.feature_extractor[5].downSample
        #
        # block4_down_sample.conv.stride = (1,1)
        # block5_down_sample.conv.stride = (1,1)
        #self.ex_channels = nn.Conv2d(256,1024,kernel_size=1,stride=1,padding=0)

    def forward(self,x):
        #x = self.feature_extractor(x)
        #x = self.ex_channels(x)
        x0,x1,x2 = self.net(x)
        return x0,x1,x2
...
class SSD300(nn.Module):
    def __init__(self, backbone=None, num_classes=21):
        ...
        # out_channels = [256, 512, 1024, 512, 256, 256] for resnet50
    ...
    def _build_additional_features(self, input_size):
        ...
        """
        additional_blocks = []
        # input_size = [256, 512, 1024, 512, 256, 256] for resnet50
        middle_channels = [256, 128, 128]
        for i, (input_ch, output_ch, middle_ch) in enumerate(zip(input_size[2:-1], input_size[3:], middle_channels)):
            padding, stride = (1, 2) if i < 1 else (0, 1)
            layer = nn.Sequential(
    ...
    def forward(self, image, targets=None):
        ...
        # Feature Map 38x38x256, 19x19x512, 10x10x1024, 5x5x512, 3x3x256, 1x1x256
        detection_features = torch.jit.annotate(List[Tensor], [])  # [x]
        for xx in x:
            detection_features.append(xx)
        x = x[-1] #3 --> 1
